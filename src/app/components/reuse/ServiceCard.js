...
const ServiceCard = () => {
    ...
    return (
        <section className="bg-gray-900 flex flex-col gap-4 justify-center  w-full py-16">
            ...
            <div className="flex flex-wrap space-x-2 space-y-3 md:space-x-4 md:space-y-4 md:px-6 px-3 justify-center w-full ">
                {["Cloud Advisory", "Cloud Migration", "Application Modernization", "Data & Analytics",
                ].map((item, index) => (
                    <span key={index}
                        onClick={() => setSelectService(item)}
                        className={`${selectService === item && 'bg-gray-500'} 
                        rounded-full mx-4 px-4 py-3 border border-blue-100 cursor-pointer hover:border-blue-200 hover:bg-gray-600`}>
                        {item}
                    </span>
                ))
                }
